...
public class AgentJar {
	...
	private static Class<AgentLoaderInterface> loadAgentLoaderClass(){
		try {
			...
			try {
				String agentLoaderClassResource = "/" + AgentHotSpotClass.replace(".", "/") + ".class";
				return ClassPathHelper.defineClass(systemLoader, AgentJar.class.getResourceAsStream(agentLoaderClassResource));
			} catch (Exception e) {
