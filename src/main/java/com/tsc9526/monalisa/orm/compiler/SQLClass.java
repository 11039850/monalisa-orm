 ...
 *******************************************************************************************/
package com.tsc9526.monalisa.orm.compiler;
...
import com.tsc9526.monalisa.orm.compiler.query.QueryPackage;
import com.tsc9526.monalisa.orm.compiler.query.QueryStatement;
...
public class SQLClass implements Closeable{
	...
	public synchronized void compile()throws Exception{
		...
		MelpFile.mkdirs(dirSrc);
		File classes=MelpFile.mkdirs(dirClasses);
		...
		if(!classFile.exists() || classFile.lastModified()<lastModified){
			...
			MelpFile.write(new File(dirSrc,className+".java"),writer.getContent().getBytes("utf-8"));
